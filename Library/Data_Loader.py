...
class DATA_1M(): 
    ...
    def loading_data(self):
        """
                Loading data is an object which return the whole dataset already reshaped, reorganized with respective label for 
                Clear, Wifi and Lte arrays data;
                
                
        """
                # Loading Data
        data = ['Clear.npy', 'WIFI.npy', 'LTE_1M.npy']
        signal = list(map(lambda x: np.load(f"Data_\{x}"), data))  # Data load



        self.row = int(np.round( (self.sec*(signal[0].shape[0]) / (3*60)) /20000 )*20000 ) # tranformation of seconds parameters given as nº of rows

        jump_time_rows = int(self.jump *(signal[0].shape[0]) / (3*60)) # jump time rows is the seconds given in the parameters into the nº rows
        empyt_list = []

        empyt_list = list(map(lambda x:[x[jump_time_rows:jump_time_rows+int(self.row/self.n_jumps)] for i in range(self.n_jumps)],
