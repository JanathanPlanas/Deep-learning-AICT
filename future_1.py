...
if __name__ == "__main__":
    ...
    test = pd.read_csv(
        r'future-1\scenario17_dev_series_val.csv')

    df_test = merge_data(test).drop(columns='blockage_final')

    blockage_final_column = building_data(test, 'unit1_pwr_60ghz_1')[
        ['blockage_final']]

    test_final = pd.concat([df_test, blockage_final_column], axis=1)

    test_final .to_parquet(r'future_clean\future-1\val.parquet')
