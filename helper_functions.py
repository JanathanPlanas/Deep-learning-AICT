...
def accuracy_fnn(y_true, y_pred):
...
class RandomizedSearchCVWrapper:
    ...
    def training_loop_no_writter(model: torch.nn.Module,
                                 data_loader_train: torch.utils.data.DataLoader,
                                 data_loader_test: torch.utils.data.DataLoader,
                                 loss_fn: torch.nn.Module,
                                 optimizer: torch.optim.Optimizer,
    ...
    def print_confusion_matrix(y_true: torch.Tensor, y_pred: torch.Tensor, num_classes: int, class_names):

        # class_names = ['BLOCK', 'CLEAR']
